...
> The same type is also exported as `TweetComponents`, which is deprecated and kept only so code written against upstream keeps compiling. Use `TwitterComponents`.
