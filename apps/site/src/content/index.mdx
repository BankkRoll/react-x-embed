...
Paste any post URL or ID to render it. This page is a static export with no
server behind it — the post is fetched in your browser with
[`useTweet`](/api-reference#usetweet), which is what `Tweet` falls back to
wherever React Server Components aren't available.
